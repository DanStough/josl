...
module.exports = class Stoplight {
  ...
  // Use one method to coordinate events for the whole stoplight
  nextSeries (events) {
    // Need to convert delays to dates here because the raspi takes too long b/n
    // processing observables to send them in series.
    let futureAlarm = this.alarmEnabled
    let beginDate = new Date((new Date()).getTime() + 200) // Pad this with 200 ms for calculation buffer

    // TODO: Remove RxJs here; there's no reason to process this using regular JS
    // synchronously. I went a little RxJs crazy
    this.eventSub = rx.Observable.from(events)
      ...
      // For events to light indicators, adjust based on alarm status
      .map((event) => {
        if (event.dest === 'ALARM') {
          futureAlarm = event.type === 'ENABLE'
        } else if (event.type === 'ENABLE' && futureAlarm && this.muted) {
          event.type = 'FLASH'
        } else if (event.type === 'FLASH' && this.alarmEnabled && !this.muted) {
          event.type = 'STEADY'
        }
        return event
      })
      // Relay to individual indicators
      .subscribe(
        (event) => {
