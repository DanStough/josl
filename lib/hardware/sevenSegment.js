...
module.exports = class SevenSegment {
  ...
  async showNumber (number, keepPoints = false) {
    ...
    await this._showDisplay(newDisplay, keepPoints = false)
  ...
  async showHello () {
    ...
    await this.writeByte(this.address, 0x30)

    await this.writeByte(this.address, 0x77)
    await this.writeByte(this.address, 0x04)
  ...
  async showSmile () {
    ...
    await this.writeByte(this.address, 0x1C)
  ...
  async showTime (time, isPM) {
    debug(this.name + ' seven segment being sent clock time')

    await this.showNumber(time, true)

    // Set Colon
    await this.writeByte(this.address, 0x77)
    if (isPM) {
      await this.writeByte(this.address, 0x18)
    } else {
      await this.writeByte(this.address, 0x10)
    }

    return true
  }

  // Synchronous I2C Methods; USE AT YOUR OWN RISK!

  _showNumberSync (number) {
    let intInput = parseInt(Math.abs(number))
    if (intInput > 9999) {
      debug(this.name + ' seven segment sent OVERFLOW')
  ...
  async _showDisplay (newDisplay, keepPoints = false) {
    // Clear points
    if (!keepPoints) {
      await this.writeByte(this.address, 0x77)
      await this.writeByte(this.address, 0x00)
